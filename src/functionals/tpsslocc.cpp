...
template<class num>
static num pbeloc_eps(const densvars<num> &d)
{
...
template<class num>
static num pbeloc_eps_pola(const num &a, const num &gaa)
{
  using xc_constants::param_gamma;
  const parameter beta0 = 0.0375;
  const parameter aa = 0.08;
...
static num C(const densvars<num> &d)
{
  ...
  num C0 = 0.35 + 0.87*pow2(d.zeta) + 0.50*pow(d.zeta,4) + 2.26*pow(d.zeta,6); // C(zeta,0)
...
// sum of mmax between fully and partially polarized PBEloc energies per particle
{
    ...
    return ( d.a*max(epsc_pbeloc,epsc_pbeloc_a) + d.b*max(epsc_pbeloc,epsc_pbeloc_b) )/d.n;
}


template<class num>
static num epsc_revpkzb(const densvars<num> &d)
{
