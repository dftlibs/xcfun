...
{
  template<class num> 
  static num get_SCAN_Fx(const num, const num, const num, const int, const int, const int);
  template<class num> 
  ...
  template<class num>
  static num scan_ec0(const num, const num, const num, 
          const parameter, const parameter, const parameter);
  template<class num>
  static num lda_0(const num, const parameter, const parameter, const parameter);
  template<class num>
  static num scan_ec1(const num, const num, const num,
          const parameter[8], const parameter,
          const parameter, const parameter, const parameter,
          const int);
  template<class num>
  ...
  template<class num>
  static num get_SCAN_Fx(const num d_n, const num d_g, const num d_tau, 
          const int IALPHA, const int IINTERP, const int IDELFX)
  {
    ...
    num s = 0.0;
    if (abs(d_g) > 1.0e-16) {
      s = sqrt(d_g) / (2.0 * pow(3.0*PI2, 1.0/3.0) * pow(d_n,4.0/3.0));  // Eq. (4)
    };
  ...
  template<class num>
  static num SCAN_X_Fx(const num s, const num alpha, const parameter ETA, const int IINTERP, const int IDELFX)
  {
    ...
    const parameter IE_PARAMS[8] = {1.0, -0.667, -0.4445555, -0.663086601049, 1.451297044490, -0.887998041597, 0.234528941479, -0.023185843322};
    ...
    if (IDELFX == 2) {
        ...
        num C_pp = 146.0/2025.0*pow2(eta_term) - 73.0/405.0*eta_term + pow2(ALPHA_GE*C2 + MU)/K1;
        ...
        num damp_4_t2 = exp(-pow2(oma)/pow2(DX_DAMP4_A) - pow2(p)/pow(DX_DAMP4_P, 4));
  ...
  template<class num>
  static num SCAN_C( const densvars<num> &d, const int IALPHA, const int IINTERP, const int IDELEC ) 
  {
    ...
    const parameter IE_PARAMS[8] = {1.0, -0.64, -0.4352, -1.535685604549, 3.061560252175, -1.915710236206, 0.516884468372, -0.051848879792};
  ...
  template<class num>
  static num scan_ec0(const num rs, const num s, const num zeta, 
          const parameter B1C, const parameter B2C, const parameter B3C)
  {
  ...
  template<class num>
  static num lda_0(const num rs, const parameter B1C, const parameter B2C, const parameter B3C)
  {
  ...
  template<class num>
  static num scan_ec1(const num rs, const num s, const num zeta,
          const parameter IE_PARAMS[8], const parameter ETA,
          const parameter B1C, const parameter B2C, const parameter B3C,
          const int IDELEC)
  {
  ...
  template<class num>
  static void get_lsda1(const num rs, const num sqrtrs, const num zeta, num &eclda1, num &d_eclda1_drs)  {
    const parameter GAM = 0.51984209978974632953442121455650;
    const parameter FZZ = 8.0/(9.0*GAM);
    const parameter p_eu[6] = {0.03109070, 0.213700, 7.59570, 3.58760, 1.63820, 0.492940};
    const parameter p_ep[6] = {0.015545350, 0.205480, 14.11890, 6.19770, 3.36620, 0.625170};
    const parameter p_alfm[6] = {0.01688690, 0.111250, 10.3570, 3.62310, 0.880260, 0.496710};
    ...
    d_eclda1_drs = (1.0 - z4*f)*deudrs + z4*f*depdrs - (1.0 - z4)*f*dalfmdrs/FZZ;
    return;
  }

  template<class num>
  static void gcor2(const parameter P[6], const num rs, const num sqrtrs, num &GG, num &GGRS)
  {
    ...
    num Q1 = 2.0*P[A]*sqrtrs*(P[B1] + sqrtrs*(P[B2] + sqrtrs*(P[B3] + P[B4]*sqrtrs)));
    num Q1RS = P[A]*(2.0*P[B2] + P[B1]/sqrtrs + 3.0*P[B3]*sqrtrs + 4.0*P[B4]*rs);
