#ifndef PW92EPS_H
#define PW92EPS_H

#ifndef CONSTANTS_H
#include "constants.h"
#endif

namespace pw92eps
{
  template<class num>
  static num e(const num &r, const parameter t[7])
  {
    return -2*t[0]*(1+t[1]*r)*
      log(1+0.5/(t[0]*(t[2]*sqrt(r)+t[3]*r+t[4]*
		       pow(r,3.0/2)+t[5]*pow(r,t[6]+1))));
  }

  template<class num>
  static num eopt(const num &sqrtr, const parameter t[6])
  {
    return -2*t[0]*(1+t[1]*sqrtr*sqrtr)*
      log(1+0.5/(t[0]*(sqrtr*(t[2]
			      +sqrtr*(t[3]
				      +sqrtr*(t[4]
					      +t[5]*sqrtr))))));
  }

  template<class num>
  static num omega(const num &z)
  {
    return (pow(1+z,4.0/3.0)+pow(1-z,4.0/3.0)-2)/(2*pow(2,1.0/3.0)-2);
  }

  template<class num>
  static num eps(const densvars<num> &d)
  {
    // The T..P parameters, but transposed for easier use with the e function
    const parameter TUVWXYP[3][7] = 
      {{0.031091, 0.21370, 7.5957 , 3.5876, 1.6382 ,0.49294,1},
       {0.015545 , 0.20548, 14.1189, 6.1977, 3.3662 ,0.62517,1},
       {0.016887, 0.11125, 10.357 , 3.6231, 0.88026,0.49671,1}};
    const parameter c = 1.709921;
    num zeta4 = pow(d.zeta,4);
    num omegaval = omega(d.zeta);
    return e(d.r_s,TUVWXYP[0])
      -  e(d.r_s,TUVWXYP[2])*omegaval*(1-zeta4)/c 
      + (e(d.r_s,TUVWXYP[1]) - e(d.r_s,TUVWXYP[0]))*omegaval*zeta4;
  }

  template<class num>
  static num eps_polarized_m0y(const num &a)
  {
    // The T..P parameters, but transposed for easier use with the e function
    const parameter TUVWXYP[3][7] = 
      {{0.031091, 0.21370, 7.5957 , 3.5876, 1.6382 ,0.49294,1},
       {0.015545 , 0.20548, 14.1189, 6.1977, 3.3662 ,0.62517,1},
       {0.016887, 0.11125, 10.357 , 3.6231, 0.88026,0.49671,1}};
    num r_s = pow(3/(4*M_PI*a),1.0/3.0);
    return e(r_s,TUVWXYP[1]);
  }

  template<class num>
  static num eps_polarized(const num &a)
  {
    // The T..P parameters, but transposed for easier use with the e function
    const parameter TUVWXYP[3][7] = 
      {{0.031091, 0.21370, 7.5957 , 3.5876, 1.6382 ,0.49294,1},
       {0.015545 , 0.20548, 14.1189, 6.1977, 3.3662 ,0.62517,1},
       {0.016887, 0.11125, 10.357 , 3.6231, 0.88026,0.49671,1}};
    const parameter c = 1.709921;
    num r_s = pow(3/(4*M_PI*a),1.0/3.0);
    num omegaval = omega(num(0));
    return e(r_s,TUVWXYP[0])
      -  e(r_s,TUVWXYP[2])*omegaval/c;
  }

  // This is with the Daresbury parameters for PBEc, not exactly
  // the same as for Daresbury pw92. 
  template<class num>
  static num eps_pbe_noopt(const densvars<num> &d)
  {
    // The T..P parameters, but transposed for easier use with the e function
    const parameter TUVWXYP[3][7] = 
      {{0.0310907, 0.21370, 7.5957 , 3.5876, 1.6382 ,0.49294,1},
       {0.01554535 , 0.20548, 14.1189, 6.1977, 3.3662 ,0.62517,1},
       {0.0168869, 0.11125, 10.357 , 3.6231, 0.88026,0.49671,1}};
    //    const parameter c = 1.709921;
    const parameter c = 8.0/(9.0*(2*pow(2,1.0/3.0)-2));
    num zeta4 = pow(d.zeta,4);
    num omegaval = omega(d.zeta);
    return e(d.r_s,TUVWXYP[0])
      -  e(d.r_s,TUVWXYP[2])*omegaval*(1-zeta4)/c 
      + (e(d.r_s,TUVWXYP[1]) - e(d.r_s,TUVWXYP[0]))*omegaval*zeta4;
  }
  // This is with the Daresbury parameters for PBEc, not exactly
  // the same as for Daresbury pw92. 
  template<class num>
  static num eps_pbe(const densvars<num> &d)
  {
    // The T..P parameters, but transposed for easier use with the e function
    const parameter TUVWXYP[3][7] = 
      {{0.0310907, 0.21370, 7.5957 , 3.5876, 1.6382 ,0.49294,1},
       {0.01554535 , 0.20548, 14.1189, 6.1977, 3.3662 ,0.62517,1},
       {0.0168869, 0.11125, 10.357 , 3.6231, 0.88026,0.49671,1}};
    //    const parameter c = 1.709921;
    const parameter c = 8.0/(9.0*(2*pow(2,1.0/3.0)-2));
    num zeta4 = pow(d.zeta,4);
    num omegaval = preexpand(omega,d.zeta);
    num sqrtr = sqrt(d.r_s);
    num e0 = preexpand(eopt,sqrtr,TUVWXYP[0]);
    return
      e0 - preexpand(eopt,sqrtr,TUVWXYP[2])*omegaval*(1-zeta4)/c 
      + (preexpand(eopt,sqrtr,TUVWXYP[1])
	 - e0)*omegaval*zeta4;
  }
}

#endif
