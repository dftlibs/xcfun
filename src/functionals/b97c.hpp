...
namespace b97c
{
    ...
    template<class num>
    static num energy_b97c_par(const parameter &Gamma, const parameter c_params[],const num &a,  const num &a_43, const num &gaa, num &e_LSDA)
    {
    ...
    template<class num>
    static num energy_b97c_antipar(const parameter &Gamma, const parameter c_params[], const densvars<num> &d, const num &e_LSDA_a, const num &e_LSDA_b)
    {
        
         num e_LSDA = pw92eps::pw92eps(d)*d.n - e_LSDA_a - e_LSDA_b;

         num s2_ab = 0.5*(pow(b97xc::spin_dens_gradient_ab2(d.gaa, d.a_43),2)+pow(b97xc::spin_dens_gradient_ab2(d.gbb, d.b_43),2));
