...
struct tfuns
{
  ...
  static void compose(T *f, const T*x)
  {
    switch(N)
      ...
      case 5:
	f[5] = 
	  f[1]*x[5] + x[1]*(2*f[2]*x[4] + 
			    x[1]*(3*f[3]*x[3] + x[1]*(4*f[4]*x[2] + 
						      f[5]*x[1]*x[1])) + 
			    3*f[3]*x[2]*x[2])
	  + 2*f[2]*x[2]*x[3];
      case 4:
	f[4] = 
	  f[1]*x[4] + x[1]*(2*f[2]*x[3] + x[1]*(3*f[3]*x[2] + f[4]*x[1]*x[1])) + 
	  f[2]*x[2]*x[2];
      case 3:
	f[3] =
	  f[1]*x[3] + x[1]*(2*f[2]*x[2] + f[3]*x[1]*x[1]);
      case 2:
...
template<class T,int N>
static void inv_expand(T *t, const T &a)
{
...
static void exp_expand(T *t, const T &x0)
{
  T ifac = 1;
  t[0] = exp(x0);
...
template<class T,int N>
static void log_expand(T *t, const T &x0)
{
...
template<class T,int N>
static void pow_expand(T *t, T x0, T a)
{
  if (x0 <= 0)
...
/* Use that (x0+x)^a=x0^a*(1+x/x0)^a */
template<class T,int N>
static void sqrt_expand(T *t, const T &x0)
{
...
template<class T,int N>
static void cbrt_expand(T *t, const T &x0)
{
  assert(x0 > 0 && "pow(x,a) not real analytic at x <= 0");
  t[0] = cbrt(x0);
  T x0inv = 1/x0;
...
template<class T,int Ndeg>
static void atan_expand(T *t, T a)
{
 ...
 */
template<class T,int Ndeg>
static void gauss_expand(T *t, const T &a)
{
...
// Taylor expand in x^2 and integrate.
template<class T,int Ndeg>
static void erf_expand(T *t, const T &a)
{
...
template<class T,int Ndeg>
static void sin_expand(T *t, const T &a)
{
    if (Ndeg > 0)
    {
...
template<class T,int Ndeg>
static void cos_expand(T *t, const T &a)
{
    if (Ndeg > 0)
    {
	T s = sin(a), c = cos(a), fac = 1;
...
template<class T,int Ndeg>
static void asinh_expand(T *t, const T &a)
{
  T tmp[Ndeg+1];
  tmp[0] = 1+a*a;
  if (Ndeg>0)
